...
namespace ArgonautDiceBot
{
    ...
    internal class Program
    {
        ...
        {
            Console.WriteLine("Starting ArgonautDiceBot (Phase 3/4)...");
...
// -------------------------
// Wait for invited planets
// -------------------------
while (client.PlanetService.JoinedPlanets.Count == 0)
{
    Console.WriteLine("Waiting for bot to be invited to a planet...");
    await Task.Delay(2000);
}


// -------------------------
// Initialize joined planets
// -------------------------
    ...
    foreach (var ch in joinedPlanet.Channels)
    {
        ...
        if (ch.ChannelType == ChannelTypeEnum.PlanetChat)
        {
            ...
            int[] ParseAndRollDetailed(string input, out int total, out string breakdown)
            {
                ...
                {
                    rolls[i] = rng.Next(1, dieSides + 1);
                    sum += rolls[i];
                }

                total = sum + modifier;
                breakdown = $"{total} ({string.Join("+", rolls)}{(modifier != 0 ? $"){(modifier > 0 ? "+" : "")}{modifier}" : "")})";
                return rolls;
            }
                ...
                try
                {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "init":
                            ...
                            if (string.IsNullOrWhiteSpace(args) || !double.TryParse(args, out double initiative))
                            {
...
case "endsession":
    ...
    break;
            ...
            Console.WriteLine("Phase 3/4 listening...");
